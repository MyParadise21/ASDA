...
class Data2VecMultiModel(BaseFairseqModel):
    ...
    def forward(
        ...
        # decode features merged with masked tokens, (batch_size * clone_batch, patch, 768)
        xs = []
        if self.shared_decoder is not None:
        ...
        with torch.no_grad():
            ...
            for i, blk in enumerate(ema_blocks):  # 12 layers Transformer
                ab = ema_alibi_bias
                if ab is not None and alibi_scale is not None:
                    scale = (
                        ema_alibi_scale[i]
        ...
        if self.cfg.clone_batch > 1:
            y = y.repeat_interleave(self.cfg.clone_batch, 0)  # (16,512,768)
        ...
        if self.cfg.cls_loss > 0 and not self.utterance_level:
            ...
            cls_pred = x[:, extra_tokens - 1] # student encoder output
