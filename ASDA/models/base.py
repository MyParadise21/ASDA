...
class ModalitySpecificEncoder(nn.Module):
    ...
    def decoder_input(self, x, mask_info: MaskInfo):
        inp_drop = self.modality_cfg.decoder.input_dropout  # 0.0
        if inp_drop > 0:
            x = F.dropout(x, inp_drop, training=self.training, inplace=True)

        num_extra = self.modality_cfg.num_extra_tokens  # 1

        if mask_info is not None:
            num_masked = mask_info.ids_restore.shape[1] - x.shape[1] + num_extra  # num_masked=410

            mask_tokens = x.new_empty(
                x.size(0),
                num_masked,
                x.size(-1),
            ).normal_(0, self.modality_cfg.mask_noise_std)  # 16,410,768

            x_ = torch.cat([x[:, num_extra:], mask_tokens], dim=1)  # 16,512,768
            x = torch.gather(x_, dim=1, index=mask_info.ids_restore)  # 16,512,768
