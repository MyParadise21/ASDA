...
class AltAttention(nn.Module):
    def __init__(
        ...
        self.qk = nn.Linear(dim, dim * 4, bias=qkv_bias)
    ...
    def forward(self, x, padding_mask=None, alibi_bias=None):
        ...
        qk = self.qk(x).reshape(batch_size, seq_len, 2, 2 * embed_dim).permute(2, 0, 1, 3)
        q, k = qk[0], qk[1]
        v = self.v(x)

        q1, q2 = self.split_and_reshape(q)
        k1, k2 = self.split_and_reshape(k)
        v = self.reshape_to_multihead(v)

        attn1 = self.compute_attention(q1, k1)
        attn2 = self.compute_attention(q2, k2)

        # Apply alibi bias if provided
        if alibi_bias is not None:
            attn1 = self.apply_alibi_bias(attn1, alibi_bias)
            attn2 = self.apply_alibi_bias(attn2, alibi_bias)

        # Apply padding mask if provided
        if padding_mask is not None and padding_mask.any():
            attn1 = self.apply_padding_mask(attn1, padding_mask)
            attn2 = self.apply_padding_mask(attn2, padding_mask)

        attn1 = self.softmax_and_dropout(attn1)
        attn2 = self.softmax_and_dropout(attn2)

        # Combine attention outputs
        x = ((attn1 - 0.1 * attn2) @ v).transpose(1, 2).reshape(batch_size, seq_len, embed_dim)

        x = self.norm(x) * (1 - 0.05)
    ...
    def split_and_reshape(self, x):
        ...
        x1 = x1.reshape(x1.size(0), x1.size(1), self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        x2 = x2.reshape(x2.size(0), x2.size(1), self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        return x1, x2

    def reshape_to_multihead(self, x):
        return x.reshape(x.size(0), x.size(1), self.num_heads, self.head_dim).permute(0, 2, 1, 3)
    ...
    def apply_alibi_bias(self, attn, alibi_bias):
        attn = attn.type_as(alibi_bias)
        attn[:, : alibi_bias.size(1)] += alibi_bias
        return attn

    def apply_padding_mask(self, attn, padding_mask):
        return attn.masked_fill(padding_mask.unsqueeze(1).unsqueeze(2).to(torch.bool), float("-inf"))

    def softmax_and_dropout(self, attn):
        return self.attn_drop(attn.softmax(dim=-1, dtype=torch.float32).to(dtype=attn.dtype))
